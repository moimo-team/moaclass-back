...
export class MomentoesService {
  ...
  async create(
    ...
    if (!imageUrl) {
    throw new BadRequestException('모멘토 등록을 위해 프로필 이미지는 필수입니다.');
  }
    ...
    try {
      ...
      if (error.code === 'P2002') {
      throw new ConflictException('이미 사용 중인 닉네임이거나 이미 등록된 유저입니다.');
    }
      throw new InternalServerErrorException(
        '모멘토 생성 중 오류가 발생했습니다.',
      );
  ...
  async update(userId: number, dto: UpdateMomentoDto, file?: Express.Multer.File) {
  // 1. 기존 프로필 존재 여부 확인
  ...
  try {
    ...
    throw new InternalServerErrorException('모멘토 수정 중 오류가 발생했습니다.');
  }
}
...
async remove(userId: number) {
  ...
  try {
    ...
    if (error.code === 'P2025') {
      throw new NotFoundException('이미 삭제되었거나 존재하지 않는 프로필입니다.');
    }
    throw new InternalServerErrorException('모멘토 삭제 중 오류가 발생했습니다.');
  }
}
